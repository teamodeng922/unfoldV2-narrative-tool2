    ...
    <div className="w-full py-4">
      <div className="relative aspect-[4/3] w-full overflow-hidden rounded-lg border border-white/[0.10] bg-[linear-gradient(135deg,#17100d,#050509_55%,#161314)]">
        ...
        {locations.map((location, index) => {
          ...
          return (
          <div key={location.id} className="absolute -translate-x-1/2 -translate-y-1/2" style={{ left: position.x, top: position.y }}>
      ...
      <div className="mt-5 grid grid-cols-2 gap-3">
        {locations.map((location, index) => (
          <article key={location.id} className="overflow-hidden rounded-lg border border-white/[0.10] bg-[#111217]">
            <div
      ...
      <div className="mt-4 flex justify-center">
        <RegenButton onClick={generateScenes}>重新生成</RegenButton>
      </div>
    </div>
  );
}
