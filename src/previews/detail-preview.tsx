    ...
    <div className="w-full py-4">
      <div className="mb-3 grid grid-cols-4 gap-2">
        {stats.map((stat, statIndex) => (
          <div key={stat.id} className="rounded-lg border border-white/[0.10] bg-[#141720] p-2 text-center">
            <p className="flex items-center justify-center gap-1.5 text-[13px] text-white/52">
      ...
      <button type="button" onClick={() => setIndex((current) => (current + 1) % scenes.length)} className="relative aspect-video w-full overflow-hidden rounded-lg border border-white/[0.10] bg-[linear-gradient(135deg,#121018,#08080d_55%,#17120f)] text-left shadow-2xl">
      ...
      <div className="mt-4 grid grid-cols-3 gap-2">
        {["选项门槛: 开", "自由多线", "修罗场: 开", "主动来访", "赠送物品", "崩盘: 关"].map((item) => (
          <div key={item} className="rounded-lg border border-white/[0.10] bg-[#111217] p-3 text-center text-[14px] text-white/62">{item}</div>
        ))}
      </div>
    </div>
  );
